...
public class SequenceFileMerger<KeyType extends WritableComparable,ValueType extends Writable> {
  ...
  // a reference to the raw writer interface if _writer implements RawDataSpillWriter
   ...
   * @throws IOException
   */
  public SequenceFileMerger(FileSystem fileSystem,
      Configuration conf,
  ...
  /**
   * construct a specialized merger that uses an optimized key generator to speed merges (used by merge sort spill writer)
   * 
   * this constructor is package private since it requires a special contract between mergesortspillwriter and sequencefilemerger 
   * 
   * @param fileSystem
   * @param conf
   * @param inputSegments
   ...
   * @throws IOException
   */
  SequenceFileMerger(FileSystem fileSystem,
      Configuration conf,
      Vector<Path> inputSegments,
      SpillWriter<KeyType,ValueType> spillWriter,
      Class<KeyType> keyClass,
  ...
  public void mergeAndSpill(Reporter reporter) throws IOException { 
    ...
    while (_segmentList.getHead() != null) {
      ...
      try { 
        ...
        // LOG.info("Spilling Segment:" + spillSegment.getName() + " Key:" + spillSegment.getKey().toString());
        // LOG.info("Spilling Segment:" + spillSegment.getName() + " Key:" + spillSegment.getKey().toString());
        ...
        if (_optionalCombiner == null) { 
          ...
          if (_optimizedKeyGenerator != null) { 
          	...
          	LOG.info("Spilling Record From Segment:" + spillSegment.getName() 
          			+ " OptKeyValue:" + spillSegment.getOptimizedKey().getLongKeyValue() 
          			+ " HeaderSize:" + spillSegment.getOptimizedKey().getHeaderSize()
          			+ " KeySize:" + (spillSegment.getRawKeyData().getLength() -  spillSegment.getOptimizedKey().getHeaderSize() - 4)
          			+ " KeyDataLength:" + spillSegment.getRawKeyData().getLength()
          			);
          	*/
          	
          	
	        	// ok segments with optimized keys have {optimized key header} + {original-key-len} preceeding the actual key bytes
          	// and optional buffer data at tail end of value 
          	
	        	_rawWriter.spillRawRecord(
	          spillSegment.getRawKeyData().getData(), 
	          spillSegment.getOptimizedKey().getHeaderSize() + 4, 
	          spillSegment.getRawKeyData().getLength() - spillSegment.getOptimizedKey().getHeaderSize() - 4,
	          spillSegment.getRawValueData().getData(), 
	          0, 
            ...
            _rawWriter.spillRawRecord(
                spillSegment.getRawKeyData().getData(), 0, spillSegment.getRawKeyData().getLength(),
                spillSegment.getRawValueData().getData(), 0, spillSegment.getRawValueData().getLength());
          }
          else { 
         ...
         if (valueBuffer.size() != 0 && lastCombinerKey.compareTo(spillSegment.getKey()) != 0) {
           ...
           // LOG.info("DEBUG:Spilling Combined Values for Key:" + lastCombinerKey.toString() + " Value Count:" + valueBuffer.size());
        ...
        if (spillSegment.next()) {
          ...
          if (_segmentList.size() == 0) { 
            ...
            MergeResultSegment<KeyType,ValueType> insertionPos = _findInsertionPos(spillSegment,sortArray,_segmentList.size());
            // if null, add to head ... 
            if (insertionPos == null) { 
              // LOG.info("DEBUG:Adding Key:" + spillSegment.getKey().toString() + " Before:" + _segmentList.getHead().getKey().toString());
              _segmentList.addHead(spillSegment);
            }
            else {
              //LOG.info("DEBUG:Adding Key:" + spillSegment.getKey().toString() + " After:" + insertionPos.getKey().toString());
        ...
        // otherwise ... 
          ...
          // close the segment 
          //LOG.info("Segment:" + spillSegment.getName() + " Exhausted. Closing");
          try { 
            spillSegment.close();
          }
          catch (IOException e) { 
            LOG.error("Segment:" + spillSegment.getName() + " Exception:" + CCStringUtils.stringifyException(e));
          }
        }
      }
      catch (IOException e) { 
        ...
        if (spillSegment != null) { 
          LOG.error("Error during splill of segment:" + spillSegment.getName() + " Exception:" + CCStringUtils.stringifyException(e));
        }
      }
    ...
    LOG.info("Merge took:" + (System.currentTimeMillis() - sortStartTime) 
        + " InputRecordCount:" + _inputRecordCount
  ...
  private void init(FileSystem fileSystem,
    ...
    try { 
      ...
      for (Path path : inputSegments) {
        ...
        MergeResultSegment<KeyType,ValueType> resultSegment = new MergeResultSegment<KeyType,ValueType>(fileSystem,conf,path,keyClass,valueClass,_rawComparator != null || _optimizedKeyGenerator != null,_optimizedKeyGenerator);
        if (!resultSegment.next()) { 
          //LOG.info("QueryResultSegment:" + path +" returned EOS on initial next.Ignoring Segment");
          try { 
            resultSegment.close();
          }
          catch (IOException e) { 
            LOG.error("QueryResultSegment:" + path +" Threw Exception:" + CCStringUtils.stringifyException(e));
          }
        }
        else {
          _inputRecordCount++;
          ...
          try { 
            if (_optimizedKeyGenerator != null) { 
              ...
              return _rawComparator.compareRaw(
                  o1.getRawKeyData().getData(),0,o1.getRawKeyData().getLength(),
                  o2.getRawKeyData().getData(),0,o2.getRawKeyData().getLength(),
                  o1.getRawValueData().getData(),0,o1.getRawValueData().getLength(),
                  o2.getRawValueData().getData(),0,o2.getRawValueData().getLength());
            }
            else { 
     ...
     if (_optimizedKeyGenerator != null){
    	 ...
    	 if (compareResult == 0 && (_optimizedKeyType & OptimizedKey.KEY_TYPE_BUFFER) != 0) { 
    		 ...
    		 compareResult = _optimizedKeyGenerator.compareOptimizedBufferKeys(
     				segment.getOptimizedKey().getBufferKeyValue().get(),
     				segment.getOptimizedKey().getBufferKeyValue().getOffset(),
     				segment.getOptimizedKey().getBufferKeyValue().getCount(),
       ...
       compareResult = _rawComparator.compareRaw(
           segment.getRawKeyData().getData(),0,segment.getRawKeyData().getLength(),
           searchSegment.getRawKeyData().getData(),0,searchSegment.getRawKeyData().getLength(),
           segment.getRawValueData().getData(),0,segment.getRawValueData().getLength(),
           searchSegment.getRawValueData().getData(),0,searchSegment.getRawValueData().getLength());
           
     }
     else { 
       compareResult = _comparator.compare(segment.getKey(), segment.getValue(), searchSegment.getKey(), searchSegment.getValue());
     }
        
     //LOG.info("Compare Between" + segment.getKey().toString() + " and " + searchSegment.getKey() + " returned:" + compareResult);
     if (compareResult > 0) {  
  ...
  private static class MergeResultSegment<KeyType extends Writable,ValueType extends Writable> extends IntrusiveListElement<MergeResultSegment<KeyType,ValueType>> { 
    ...
    public OptimizedKey getOptimizedKey() { return _optimizedKey; }
    ...
    public DataOutputBuffer getRawKeyData() { return rawKeyData; }
    public DataOutputBuffer getRawValueData() { return rawValueData; }
        ...
        if (!useRawMode) { 
          ...
          if (!eos) { 
            ...
            if (!eos && _optimizedGenerator != null) { 
            	_optimizedKey.initFromKeyValuePair(rawKeyData.getData(),0,rawKeyData.getLength(),rawValueData.getData(),0,rawValueData.getLength());
            }
          }
        }
      }
