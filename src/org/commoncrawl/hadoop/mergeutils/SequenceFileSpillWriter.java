...
public class SequenceFileSpillWriter<KeyType extends WritableComparable,ValueType extends Writable> 
  ...
  private LinkedBlockingQueue<QueuedBufferItem> _bufferQueue = new LinkedBlockingQueue<QueuedBufferItem>(BUFFER_QUEUE_CAPACITY);
  
  public SequenceFileSpillWriter(
      ...
      public void run() {
        ...
        while (true) { 
          ...
          if (queuedBufferItem._buffer == null) {
            ...
            //LOG.info("Writer Thread received item. Limit:" + theBuffer.limit());
            ...
            //LOG.info("Writer Thread Finished With Buffer. Wrote:"+ itemsWritten + " in:" + (System.currentTimeMillis() - timeStart));
          }
        }
      } 
  ...
  public void spillRecord(KeyType key, ValueType value) throws IOException {
    ...
    while (!done) {
      ...
      try {
        ...
        // LOG.info("keySizePos:" + keySizePos + " keySize:" + keySize + " valueSizePos:" + valueSizePos + " valueSize:" + valueSize);
      ...
      if (overflow) {
      	// LOG.error("Overflow Condition StartPosition:" + startPosition + " KeySizePos:" + keySizePos + " keySize:" + keySize + " valueSizePos:" + valueSizePos + " valueSize:" + valueSize);
  ...
  @Override
  public void spillRawRecord(byte[] keyData, int keyOffset, int keyLength,final byte[] valueData, final int valueOffset, final int valueLength) throws IOException {
  ...
  private void flushActiveBuffer() throws IOException { 
    try {
      if (_activeBuffer != null && _activeBuffer.position() != 0) {
        // flip the buffer ... 
        _activeBuffer.flip();
        //LOG.info("Flipped Active Buffer.Limit:" + _activeBuffer.limit()+ ". Queueing Buffer");        
  ...
  public void spillRawRecord2(byte[] keyData, int keyOffset, int keyLength,final byte[] valueData, final int valueOffset, final int valueLength) throws IOException {
    ...
    writer.appendRaw(keyData, keyOffset, keyLength, 
        new ValueBytes() {
